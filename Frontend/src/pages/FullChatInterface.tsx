...
- *"Give me a financial health checkup"*`,
    ...
    // ✅ ALTERNATIVE: Use cleanup function approach
useEffect(() => {
    const messageToSend = location.state?.sendMessage;
    
    // Only send if we have a message, user ID, and haven't sent this exact message before
    if (messageToSend && userId && autoSentRef.current !== messageToSend) {
        console.log('🚀 Auto-sending template prompt:', messageToSend);
        
        // Mark this message as sent
        autoSentRef.current = messageToSend;
...
}, [location.state?.sendMessage, userId]);
